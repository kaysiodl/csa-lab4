...
**Такты.** Одна микрокоманда = один такт
...
Поля: `TICK` — счётчик тактов; `mc` — адрес текущей микрокоманды и её значение; `IR` — имя опкода; `DR`, `TOS` — вершина стека; `Z`, `N`, `C` — флаги АЛУ; `DS` — верхние элементы стека данных; `RS` — стек возвратов
